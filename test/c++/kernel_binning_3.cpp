...
int main() {
 ...
 binning.add_dirac(1, {0, up, -2.5, 0}, {0., 1.});
 ...
 if (first_dim(values) != 2 or second_dim(values) != 3 or third_dim(values) != 2 or fourth_dim(values) != 1) return 1;

 auto ref_values = array<dcomplex, 4>(2, 3, 2, 1);
 ref_values() = 0;
 ref_values(0, 0, 0, 0) = {0, 1};
 ref_values(0, 1, 0, 0) = {2.2, -0.3};
 ref_values(1, 2, 0, 0) = {0, 1.1};
 ...
 if (first_dim(values) != 2 or second_dim(values) != 4 or third_dim(values) != 2 or fourth_dim(values) != 1) return 4;

 ref_values = array<dcomplex, 4>(2, 4, 2, 1);
 ref_values() = 0;
 ref_values(0, 0, 0, 0) = {0, 1};
 ref_values(0, 1, 0, 0) = {2.2, -0.3};
 ref_values(1, 2, 0, 0) = {0, 1.1};
 ref_values(0, 1, 1, 0) = {0.5, 0};
 ref_values(0, 2, 0, 0) = {0.6, 0};
 ref_values(0, 3, 0, 0) = {0, 10};
