...
if mpi.is_master_node():
    with HDFArchive('density40k.out.h5', 'a') as ar:  # A file to store the results
        ar['pn_values'] = pn
        ar['pn_errors'] = pn_error
        ar['sn_values'] = sn[:, index_compare]
        ar['sn_errors'] = sn_error

from pytriqs.utility.h5diff import h5diff

# Compare with density calculated from commit 63cfc21cf5211572c6ffbec7093654e2e219b63a
with HDFArchive('density40k.ref.h5', 'r') as ar:
    print "sn out:", sn[:, index_compare]
    print "sn ref:", ar['sn_values']
    print "rel diff:", np.abs((sn[:, index_compare] - ar['sn_values'])/ar['sn_values']).mean()
    print "pn out:", pn
    print "pn ref:", ar['pn_values']
    print "rel diff:", np.abs((pn - ar['pn_values'])/ar['pn_values']).mean()

h5diff("density40k.out.h5","density40k.ref.h5")
