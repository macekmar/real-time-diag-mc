...
// ---------- 
 void accumulate(dcomplex sign) {
  ...
  histogram_pn << k;
  //we only keep the number of + signs, then the number of - signs is just pn(i) - number(+ signs)
...
// ---------- 
 void collect_results(mpi::communicator c) {
   ...
   int number_points = histogram_pn_full.n_data_pts();
   //Computing the average and error values
   //For pn
   for (int i = 0; i < size_n ; i++)
   {
           pn(i) = data_histogram_pn(i)/number_points; //Average
           sn(i) = 2*(data_histogram_sn(i)/(data_histogram_pn(i))) - 1;

           //FIXME : explicit formula for the error bar jacknife of a series of 0 and 1
           pn_errors(i) = (1/double(pow(number_points,2))) * sqrt((number_points - 1) * data_histogram_pn(i) * (number_points - data_histogram_pn(i)));
           
           //FIXME : explicit formula as well for the error bar of the sn using a jacknife
           sn_errors(i) = (2/double(pow(data_histogram_pn(i),2))) * sqrt((data_histogram_pn(i) - 1) * data_histogram_sn(i)* (data_histogram_pn(i) - data_histogram_sn(i)));
