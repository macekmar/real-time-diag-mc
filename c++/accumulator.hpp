...
// There are two types: one for single time measure, one or multi time measure

class multi_accumulator {

 private:
 separated_integrand* integrand; // for multitime, the integrand must be separated
 array<double, 1>& pn;
 array<dcomplex, 2>& sn;
 ...
 public:
 // ----------
 multi_accumulator(separated_integrand* integrand, array<double, 1>* pn, array<dcomplex, 2>* sn, array<double, 1>* pn_errors,
 ...
 void accumulate(dcomplex sign) {
  integrand->measure_evaluate();

  histogram_pn << integrand->perturbation_order;
  sn_node(integrand->perturbation_order, range()) += integrand->get_measure_value() / std::abs(integrand->weight->value);
 }
...
class single_accumulator {
 ...
 public:
 // ----------
 single_accumulator(Integrand* integrand, array<double, 1>* pn, array<dcomplex, 2>* sn, array<double, 1>* pn_errors,
                    array<double, 1>* sn_errors, int* nb_measures)
    : integrand(integrand),
      pn(*pn),
      sn(*sn),
 ...
 void accumulate(dcomplex sign) {
  histogram_pn << integrand->perturbation_order;
  sn_node(integrand->perturbation_order, 0) += sign;
 }
 ...
 void collect_results(mpi::communicator c) {
  ...
  for (int k = 0; k < size_n; k++) {
   pn(k) = data_histogram_pn(k) / nb_measures; // Average
   sn(k, 0) = sn(k, 0) / data_histogram_pn(k);
