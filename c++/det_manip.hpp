...
namespace triqs { namespace modified_det_manip {
 ...
 template <typename FunctionType> class det_manip {
  ...
  using range = arrays::range;
    ...
    det_type det;
    size_t Nmax, N;
    enum {NoTry, Insert, Remove, ChangeCol, ChangeRow, ChangeRowCol, Insert2 = 10, Remove2 = 11, Refill = 20}
...
//------------------------------------------------------------------------------------------
 ...
 public:
	 ...
	 auto det_d = arrays::determinant(d);

	 value_type det_ratio;
	 if  (std::abs(det_d) < 1e-14) det_ratio = arrays::determinant(Cprimeb) * arrays::determinant(cBprime);
	 else det_ratio = det_d * arrays::determinant(Dprime - Cprime_a_Bprime + Cprimeb * inverse(d) * cBprime);
	 ...
	 value_type det_ratio;
	 det_ratio = d * arrays::determinant(Dprime - Cprime_a_Bprime + Cprimeb * cBprime / d);
	 ...
	 //  det_ratio = (Cprimeb * comat * cBprime)(0, 0);
	 //}
	 det_ratio = det_d * (Dprime - Cprime_a_Bprime(0, 0) + (Cprimeb * inverse(d) * cBprime)(0, 0));
   ...
   private :
    ...
    void check_mat_inv(double precision_warning = 1.e-5, double precision_error = 1.e-3) {
